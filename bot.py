...
# OpenAI GPT
# ------------------------------------------------------------------
async def ask_gpt(soru: str) -> str:
    if not OPENAI_API_KEY:
        raise RuntimeError("OPENAI_API_KEY ortam degiskeni ayarlanmamis.")
    ...
    def _fetch():
        ...
        if resp.status_code != 200:
            try:
                ...
                detail = resp.text
            raise RuntimeError(f"OpenAI hatasi ({resp.status_code}): {detail}")
        choices = resp.json().get("choices") or []
        if not choices:
            return "GPT bir cevap dondurmedi."
        text = ((choices[0].get("message") or {}).get("content") or "").strip()
        return text or "GPT bos bir cevap dondurdu."
...
# (baslangic saniyesi, gosterilecek asama)
PROGRESS_STAGES = [
    (0.0, "📤 Gönderiliyor"),
    (2.5, "🔎 Araştırılıyor"),
    (6.0, "🧠 Çözümleniyor"),
    (10.0, "✍️ Cevap yazılıyor"),
]


def _stage_for(elapsed: float) -> str:
    stage = PROGRESS_STAGES[0][1]
    for start_s, text in PROGRESS_STAGES:
        if elapsed >= start_s:
            stage = text
    return stage
...
async def _animate_progress_bar(message: discord.Message, soru: str):
    """GPT'den gercek cevap gelene kadar asamalari ve cubugu gosterir.
    Cevap tek seferde geldigi icin (streaming degil) asamalar zamana
    dayali bir tahmindir."""
    try:
        for i in range(1, PROGRESS_BAR_SLOTS):
            await asyncio.sleep(1.3)
            stage = _stage_for(i * 1.3)
            filled = "█" * i
...
async def make_ticket_transcript(channel: discord.TextChannel) -> discord.File:
    lines = [
        f"Ticket: {channel.name}",
        f"Kanal ID: {channel.id}",
        f"Olusturma: {channel.created_at.isoformat()}",
        "=" * 70,
    ]
    try:
        ...
        messages = []
        lines.append(f"Mesaj gecmisi okunamadi: {e}")
    for m in messages:
        stamp = m.created_at.strftime("%Y-%m-%d %H:%M:%S UTC")
        content = m.content or "[metin yok]"
        if m.attachments:
            content += " | Ekler: " + " ".join(a.url for a in m.attachments)
        lines.append(f"[{stamp}] {m.author} ({m.author.id}): {content}")
    return discord.File(io.BytesIO("\n".join(lines).encode("utf-8")), filename=f"{channel.name}-transkript.txt")
...
class TicketPanelView(discord.ui.View):
    ...
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        log_channel_id = entry.get("ticket_log_channel_id")
        log_channel = interaction.guild.get_channel(log_channel_id) if log_channel_id else None
        if log_channel:
            log_embed = discord.Embed(title="🎫 Ticket Oluşturuldu", color=0x53FC18, timestamp=datetime.now(timezone.utc))
            log_embed.add_field(name="Sahip", value=interaction.user.mention, inline=True)
            log_embed.add_field(name="Kanal", value=channel.mention, inline=True)
            try:
                await log_channel.send(embed=log_embed)
            except discord.DiscordException as e:
                print(f"[HATA] Ticket açılış logu gönderilemedi: {e}")
...
class TicketCloseView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not is_staff(interaction.user) and owner_id != str(interaction.user.id):
            ...
            return
        await interaction.response.send_message("🔒 Ticket kapatılıyor ve transkript hazırlanıyor...", ephemeral=True)
        channel = interaction.channel
        transcript = await make_ticket_transcript(channel)

        log_channel_id = entry.get("ticket_log_channel_id")
        log_channel = interaction.guild.get_channel(log_channel_id) if log_channel_id else None
        if log_channel:
            log_embed = discord.Embed(title="🔒 Ticket Kapatıldı", color=0xFF5C5C, timestamp=datetime.now(timezone.utc))
            owner = interaction.guild.get_member(int(owner_id)) if owner_id and owner_id.isdigit() else None
            log_embed.add_field(name="Sahip", value=owner.mention if owner else (f"`{owner_id}`" if owner_id else "Bilinmiyor"), inline=True)
            log_embed.add_field(name="Kapatan", value=interaction.user.mention, inline=True)
            log_embed.add_field(name="Kanal", value=f"`{channel.name}`", inline=False)
            try:
                await log_channel.send(embed=log_embed, file=transcript)
            except discord.DiscordException as e:
                print(f"[HATA] Ticket logu gonderilemedi: {e}")
        ...
        try:
            ...
            print(f"[HATA] Ticket kanalı silinemedi: {e}")
...
@tree.command(name="ticketlogkanali", description="🎫 Ticket açılış/kapanış loglarının gideceği kanalı ayarlar")
@app_commands.describe(kanal="Ticket loglarının gideceği kanal")
@app_commands.checks.has_permissions(manage_guild=True)
async def ticketlogkanali(interaction: discord.Interaction, kanal: discord.TextChannel):
    data, entry = get_feature_entry(interaction.guild_id)
    entry["ticket_log_channel_id"] = kanal.id
    save_data(data)
...
# ------------------------------------------------------------------
@tree.command(name="duyurukanaliayarla", description="📢 Bu sunucunun global duyuru kanalını ayarlar")
@app_commands.describe(kanal="Global duyuruların gideceği kanal")
@app_commands.checks.has_permissions(manage_guild=True)
async def duyurukanaliayarla(interaction: discord.Interaction, kanal: discord.TextChannel):
    data, entry = get_feature_entry(interaction.guild_id)
    entry["announcement_channel_id"] = kanal.id
    save_data(data)
    await interaction.response.send_message(f"📢 Global duyuru kanalı {kanal.mention} olarak ayarlandı.", ephemeral=True)


@tree.command(name="duyuru", description="📢 Botun bulunduğu tüm sunuculara duyuru gönderir")
@app_commands.describe(mesaj="Tüm sunuculara gönderilecek duyuru")
async def duyuru(interaction: discord.Interaction, mesaj: str):
    if interaction.guild is not None or BOT_OWNER_ID == 0 or interaction.user.id != BOT_OWNER_ID:
        await interaction.response.send_message("❌ Bu komut yalnızca bot sahibinin DM'inden kullanılabilir.", ephemeral=True)
        return

    await interaction.response.defer(ephemeral=True, thinking=True)
    data = load_data()
    sent = skipped = failed = 0
    embed = discord.Embed(title="📢 GLOBAL DUYURU", description=mesaj, color=0x5865F2, timestamp=datetime.now(timezone.utc))
    embed.set_footer(text="Kingo • Global Duyuru Sistemi")
    ...
    for guild in list(client.guilds):
        ...
        try:
            ...
            failed += 1
            print(f"[UYARI] Duyuru {guild.name} sunucusuna gonderilemedi: {e}")

    save_data(data)
    await interaction.followup.send(
        f"📢 Duyuru tamamlandı.\n✅ Gönderildi: **{sent}**\n⏭️ Kanal ayarlanmamış: **{skipped}**\n❌ Başarısız: **{failed}**",
        ephemeral=True,
    )


@tree.command(name="topludm", description="📩 Bu sunucudaki üyelere bot üzerinden özel mesaj gönderir")
@app_commands.describe(mesaj="Sunucudaki üyelere gönderilecek özel mesaj")
async def topludm(interaction: discord.Interaction, mesaj: str):
    """Bot sahibinin, bulunduğu sunucudaki üyelere kontrollü toplu DM göndermesini sağlar.

    Sadece BOT_OWNER_ID kullanabilir. Bot hesapları atlanır. Discord'un DM/rate-limit
    sınırlarına takılmamak için mesajlar arasında kısa bir bekleme uygulanır.
    """
    if interaction.guild is None:
        await interaction.response.send_message("❌ Bu komut bir sunucunun içinde kullanılmalı.", ephemeral=True)
        return
    ...
    mesaj = mesaj.strip()
    if not mesaj:
        await interaction.response.send_message("❌ Gönderilecek mesaj boş olamaz.", ephemeral=True)
        return
    if len(mesaj) > 2000:
        await interaction.response.send_message("❌ DM mesajı en fazla 2000 karakter olabilir.", ephemeral=True)
        return

    guild = interaction.guild
    await interaction.response.defer(ephemeral=True, thinking=True)

    # Members intent açık olduğunda öncelikle cache kullanılır; eksik üyeler için
    # Discord'dan tamamı çekilir.
    members = list(guild.members)
    try:
        if guild.chunked is False:
            members = [m async for m in guild.fetch_members(limit=None)]
    except (discord.Forbidden, discord.HTTPException) as e:
        print(f"[UYARI] {guild.name} üyeleri fetch edilemedi, cache kullanılacak: {e}")
    ...
    for member in members:
        ...
        try:
            ...
            # Kullanıcının DM'leri kapalıysa veya bot engelliyse.
            failed += 1
        except discord.HTTPException as e:
            failed += 1
            print(f"[UYARI] {guild.name} / {member} DM gönderilemedi: {e}")

        # Toplu DM işlemini kontrollü tutmak ve Discord API rate limitlerine
        # gereksiz yere yüklenmemek için mesajlar arasında bekle.
    ...
    await interaction.followup.send(
        ...
        f"🤖 Bot hesapları atlandı: **{skipped}**",
        ephemeral=True,
    )
