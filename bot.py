...
# Kayit sistemi (isim, yas, Roblox kullanici adi)
# ------------------------------------------------------------------
@tree.command(name="kayit", description="Isim, yas ve Roblox kullanici adinla kayit olursun")
@app_commands.describe(
    isim="Adin",
    yas="Yasin",
    roblox_kullanici_adi="Roblox kullanici adin (roblox.com/users/... degil, sadece kullanici adi)",
)
async def kayit(interaction: discord.Interaction, isim: str, yas: int, roblox_kullanici_adi: str):
    await interaction.response.defer(ephemeral=True, thinking=True)

    if yas < 1 or yas > 120:
        await interaction.followup.send("Gecerli bir yas gir.", ephemeral=True)
        return

    try:
        roblox_user = await get_roblox_user(roblox_kullanici_adi.strip())
    except Exception as e:
        await interaction.followup.send(f"Roblox bilgisi alinamadi, birazdan tekrar dene: {e}", ephemeral=True)
        return

    if not roblox_user:
        await interaction.followup.send(
            f"**{roblox_kullanici_adi}** adinda bir Roblox kullanicisi bulunamadi. "
            f"Kullanici adini kontrol edip tekrar dene.",
            ephemeral=True,
        )
        return
    ...
    embed = discord.Embed(
        ...
        description=f"**{isim}** ({yas}) olarak kaydedildin.",
        color=0x53FC18,
        timestamp=datetime.now(timezone.utc),
    )
    ...
    embed.add_field(name="Roblox hesap yasi", value=format_roblox_created(roblox_user.get("created")), inline=True)
    if roblox_user.get("is_banned"):
        embed.add_field(name="⚠️ Dikkat", value="Bu Roblox hesabi banli gorunuyor.", inline=False)
